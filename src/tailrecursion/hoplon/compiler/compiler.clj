      ...
      (if-not line
        ...
        (if-not txt
          (if (re-find start line)
            (recur [] i lines (conj out (str/replace line start "")))
            (recur txt i lines (conj out line)))
          (if (re-find end line)
            (recur nil 0 lines (conj out (pr-str (str/join "\n" txt))))
...
(defn make-nsdecl
  ...
  (let [ns-sym  (symbol ns-sym)
        ...
        other?  #(not (contains? #{:require :require-macros :refer-hoplon} (first %)))
